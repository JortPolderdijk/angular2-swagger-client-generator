...
var Generator = (function () {
  ...
  Generator.prototype.initialize = function () {
    ...
    this.templates = {
      'class': fs.readFileSync(path.join(__dirname, '/../templates/angular2-service.mustache'), 'utf-8'),
      'model': fs.readFileSync(path.join(__dirname, '/../templates/angular2-model.mustache'), 'utf-8'),
      'models_export': fs.readFileSync(path.join(__dirname, '/../templates/angular2-models-export.mustache'), 'utf-8')
    }
  ...
  Generator.prototype.createMustacheViewModel = function () {
      ...
      _.forEach(api, function (op, m) {
        ...
        if (op.responses && op.responses['200'] && op.responses['200'].schema) {
          ...
          if (schema.type) {
            ...
            var refType2 = schema['$ref']
            method.returns = refType2.substring(refType2.lastIndexOf('/') + 1)
          }
        }
        ...
        _.forEach(params, function (parameter) {
          ...
          } else if (parameter.in === 'formData') { parameter.isFormParameter = true }

          method.parameters.push(parameter)
        })

        if (method.parameters.length > 0) { method.parameters[method.parameters.length - 1].last = true }
    ...
    _.forEach(swagger.definitions, function (defin, defVal) {
      ...
      _.forEach(defin.properties, function (propin, propVal) {
        ...
        if (property.isArray) { property.type = _.has(propin.items, '$ref') ? that.camelCase(propin.items['$ref'].replace('#/definitions/', '')) : propin.items.type } else {
          property.type = _.has(propin, '$ref') ? that.camelCase(propin['$ref'].replace('#/definitions/', '')) : propin.type
        }

        if (property.type === 'integer' || property.type === 'double') { property.typescriptType = 'number' } else {
          property.typescriptType = property.type
        }

        if (property.isRef) { definition.refs.push(property) } else { definition.properties.push(property) }
      })
  ...
  Generator.prototype.getPathToMethodName = function (m, path) {
    ...
    segments = _.transform(segments, function (result, segment) {
      if (segment[0] === '{' && segment[segment.length - 1] === '}') { segment = 'by' + segment[1].toUpperCase() + segment.substring(2, segment.length - 1) }
